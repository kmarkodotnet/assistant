...
RUN dotnet restore src/FamilyOs.Workers/FamilyOs.Workers.csproj
COPY . .
RUN dotnet publish src/FamilyOs.Workers/FamilyOs.Workers.csproj -c Release -o /app/publish --no-restore
